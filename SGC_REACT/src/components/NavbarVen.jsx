...
export function NavbarVen() {
  ...
  const handleLogout = async () => {
    // Obtenemos los tokens usando las claves que definiste en Login
    const refreshToken = localStorage.getItem("refresh");
    const accessToken = localStorage.getItem("access");

    try {
        // 1. Avisar al servidor (para invalidar el token en la lista negra/Blacklist)
        // Nota: Asegúrate de que tu backend acepte la key 'refresh' o 'refresh_token' en el JSON
    ...
    // 3. Redirigir al login (Usamos /login porque es una SPA de React)
              ...
              <div className="text-right">
                <p className="text-sm text-white font-bold uppercase">
                  {loading 
                    ? "Cargando..." 
                    : salesWomanData 
                    ? salesWomanData.email // Muestra el email
                    : "No identificado"}
                </p>
      ...
      >
        <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="hidden md:flex items-center h-12 space-x-8">
            <a href='/registro-retazos' className="text-gray-900 hover:text-[#ec4444] px-3 py-2 rounded-md text-sm font-bold  transition">
              Registro de retazos 
            </a>
            ...
            <a href='/consulta-ven' className="text-gray-900 hover:text-[#ec4444] px-3 py-2 rounded-md text-sm font-bold  transition">
              Consulta de Retazos
            </a>
            <a href='#' className="text-gray-900 hover:text-[#ec4444] px-3 py-2 rounded-md text-sm font-bold border-b-2 border-transparent  transition">
            </a>
          ...
          <div className="flex md:hidden items-center justify-around h-14 w-full bg-white/80 backdrop-blur-sm -mx-4 px-12 border-t border-gray-200">
            ...
            {/* Icono Vendedoras */}
            <a href="/registro-retazos" className="flex flex-col items-center justify-center text-gray-600 hover:text-[#ec4444]">
            ...
            {/* Icono Retazos */}
            <a href="/consulta-ven" className="flex flex-col items-center justify-center text-gray-600 hover:text-[#ec4444]">
